...
from rcnn_with_fl.config import config
from rcnn_with_fl.operator_py.focal_loss import *
...
def get_resnet_train(num_classes=config.NUM_CLASSES, num_anchors=config.NUM_ANCHORS):
    ...
    # ---- data structure adapting for focal loss ----
    # rpn_cls_score_reshape_ = mx.symbol.Reshape(
    #     data=rpn_cls_score_reshape, shape=(-3,-3), name="rpn_cls_score_reshape_")
    # rpn_cls_score_swap = mx.sym.swapaxes(data=rpn_cls_score_reshape_,dim1=0,dim2=1,name='rpn_cls_score_swap')
    # ---- data structure adapting for focal loss ----

    # classification
    # rpn_cls_prob = mx.symbol.SoftmaxOutput(data=rpn_cls_score_reshape, label=rpn_label, multi_output=True,
    #                                        normalization='valid', use_ignore=True, ignore_label=-1, name="rpn_cls_prob")
    rpn_cls_prob = mx.sym.Custom(op_type='FocalLoss', name='rpn_cls_prob', data=rpn_cls_score_reshape, labels=rpn_label, gamma= 2, alpha = 0.25, use_ignore = False, ignore_label=-1)
    ...
    # cls_prob_ = mx.symbol.SoftmaxOutput(name='cls_prob', data=cls_score, label=label, normalization='batch')
    # cls_prob_ = mx.sym.SoftmaxActivation(data=cls_score, name='cls_prob_') #, mode='channel')
    cls_prob_ = mx.sym.softmax(data=cls_score, name='cls_prob_', axis=1)
    cls_prob = mx.sym.Custom(data=cls_prob_, labels=label, op_type='FocalLoss', name='cls_prob', gamma= 2, alpha = 0.25, use_ignore = False, ignore_label=-1)
