...
for a in `seq 2 2 100`;do
epoch=$a
map=0
log_file='/workspace/blued/log-test/blued_v9.1_on_0920_test.log'
touch $log_file
echo '==> epoch:'$epoch >> $log_file 
for i in $names;do
ap=0;
python -u test.py --dataset Blued --image_set 0920-$i --root_path /workspace/blued/rcnn-cache/test/ --dataset_path /workspace/dataset/blued/blued_0920/ --network resnet --prefix /workspace/blued/model/blued_v9.1_resnet/blued-v9.1 --epoch $a --thresh 0.8 | grep -v 'non' | grep $i | tail -n 1 >> $log_file;
