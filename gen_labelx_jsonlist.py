...
def _init_():
    ...
    Usage:
        gen_labelx_jsonlist.py          <in-file> <out-list> [-c|--classification]
                                        [-d|--detection -l|--clustering]
                                        [--prefix=str --sub-task=str --json=str]
    ...
    Options:
        ...
        --json=str                      optional pre-annotation json, required under clusering task.
    '''
    print('=' * 80 + '\nArguments submitted:')
...
def generate_dict(filename, prefix, classification=False, detection=False, clustering=False, sub_task=None, pre_ann=None):
    temp = dict()
...
def main():
    ...
    pre_ann = json.load(open(args['--json'],'r')) if args['--json'] else None
    with open(args['<in-file>'], 'r') as f:
        file_lst = list()
        for buff in f:
